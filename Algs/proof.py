...
data1 = df[df['machine_id'] == 'm_2259']
data2 = df[df['machine_id'] == 'm_3068']
...
# # 绘制 data2 与 time 的关系
# plt.plot(data2['time_stamp'], data2['cpu_util_percent'], label='Data 2', marker='s', linestyle='--', color='g', markersize=3)
...
# 左边两张小图
axs[0, 0].plot(data1['time_stamp'], data1['cpu_util_percent'], label='Data 1', marker='o', linestyle='-', color='b', markersize=3)
axs[0, 0].set_title("Plot 1: m_2259")
axs[0, 0].legend()

axs[1, 0].plot(data2['time_stamp'], data2['cpu_util_percent'], label='Data 2', marker='s', linestyle='--', color='g', markersize=3)
axs[1, 0].set_title("Plot 2: m_3068")
axs[1, 0].legend()

# 右边单独的一张图
axs[0, 1].plot(data_sum['time_stamp'], data_sum['value_sum'], label='Data1 + Data2', marker='^', linestyle='-.', color='r', markersize=3)
axs[0, 1].set_title("Plot 3: m_2259_m_3068")
axs[0, 1].legend()

# 隐藏下方不需要的子图 (axs[1,1] - 右下角子图)
axs[1, 1].axis('off')

plt.savefig('proof-test2.png')
